  ...
  <div class="register-wrap">
    <div class="container">
      ...
      <div class="register-form" style="margin: 0 auto;">
        <h3 class="title">手机号找回
          <router-link class="email-pwd" to="/EmailFind">邮箱找回&nbsp;></router-link>
        </h3>
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="form-pannel show">
          ...
          <el-form-item prop="imgCode">
            ...
            <a  v-if="!ruleForm.mobile" class="send-code-btn">发送验证码</a>
            <a  v-if="isT" class="send-code-btn" v-text="countdown" style="width: 61px;text-align: center;font-size: 14px"></a>
            <a @click="sent()" v-if="ruleForm.mobile && !isT" class="send-co-btn">发送验证码</a>
          </el-form-item>
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      handleBlur() {
        if (this.ruleForm.mobile.length != 0 && this.ruleForm.code.length != 0 && this.ruleForm.newPwd.length != 0 && this.ruleForm.confirmPwd.length != 0) {
          this.bg = true;
          this.flag = true;
        } else {
          this.bg = false
      ...
      //发送短信
      sent() {
        if (!until.isPhoneNum(this.ruleForm.mobile)) {
          this.$message({
            showClose: true,
            duration: 4000,
            message: '输入的账号格式有误！',
        ...
        try {
          let formData = {
            "username": this.ruleForm.mobile, //账户
          }
          this.$axios({
            method: "post",
